      ...
      with torch.no_grad():
         with model.ema_scope():
            ...
            if full_ddim == True:
               ...
               z_purified, _ = DDIM_Sampler.sample(S=ddim_num_iters, batch_size=z.shape[0], shape=z.shape[1:], conditioning=None, verbose=False, x_T=z_noisy, eta=ddim_eta, custome=True, initial_timestep=ddim_timestep)
            else:
               # Predict z_0 from z_t using Tweedie's formula (DCDP-LDM-II)
               predicted_noise = model.apply_model(z_noisy,torch.tensor(ddim_timestep-1).unsqueeze(0).to(device),cond=None)
